...
#define S_LOG_DEBUG(hostID, accountUID, fmt, ...) \
    { { jy::Logger::GetInstance().Debug(JY_TEXT("hostID:{} accountUID:{} " fmt), hostID, accountUID, ##__VA_ARGS__); } }

#define S_LOG_INFO(hostID, accountUID, fmt, ...) \
    { { jy::Logger::GetInstance().Info(JY_TEXT("hostID:{} accountUID:{} " fmt), hostID, accountUID, ##__VA_ARGS__); } }

#define S_LOG_WARN(hostID, accountUID, fmt, ...) \
    { { jy::Logger::GetInstance().Warn(JY_TEXT("hostID:{} accountUID:{} " fmt), hostID, accountUID, ##__VA_ARGS__); } }

// S_LOG_ERROR는 __FUNCTION__ (현재 함수 이름)을 추가
#define S_LOG_ERROR(hostID, accountUID, fmt, ...) \
    { { jy::Logger::GetInstance().Error(JY_TEXT("hostID:{} accountUID:{} [{}] " fmt), hostID, accountUID, static_cast<const char*>(__FUNCTION__), ##__VA_ARGS__); } }

#define S_LOG_CRITICAL(hostID, accountUID, fmt, ...) \
    { { jy::Logger::GetInstance().Critical(JY_TEXT("hostID:{} accountUID:{} [{}] " fmt), hostID, accountUID, static_cast<const char*>(__FUNCTION__), ##__VA_ARGS__); } }
