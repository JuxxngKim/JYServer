...
namespace util
{
	...
	{
		const SDateTime changeTime{ year, month, 0, day, hour, minute, second, 0 };
	...
	SDateTime TimeUtil::FindSystemKSTDate() const
	{
		...
		SDateTime result = ConvertTmToSDateTime(tm, milliSec);

		return result;
	}

	SDateTime TimeUtil::FindKSTDate() const
	{
	...
	int64_t TimeUtil::FindIntervalSeconds(const SDateTime& start, const SDateTime& end) const
	{
	...
	bool TimeUtil::NeedToDailyRenewal(const int64_t& befMs, const int64_t& curMs, const int32_t& resetHour)
	{
		...
		return nextDailyResetTime <= ConvertKSTTimeMsToSDateTime(curMs);
	}
	...
	bool TimeUtil::NeedToWeeklyRenewal(const int64_t& befMs, const int64_t& curMs, const int32_t& resetHour, const int32_t& resetDayOfWeek)
	{
		...
		return nextWeeklyResetTime <= ConvertKSTTimeMsToSDateTime(curMs);
	}
	...
	SDateTime TimeUtil::AddHourSDateTimeToKSTDate(const SDateTime& time, const int32_t& addHour) const
	{
		return AddSecondsSDateTimeToKSTDate(time, std::chrono::seconds(std::chrono::hours(addHour)).count());
	}

	SDateTime TimeUtil::AddDaySDateTimeToKSTDate(const SDateTime& time, const int32_t& addDay) const
	{
		return AddSecondsSDateTimeToKSTDate(time, std::chrono::seconds(std::chrono::hours(addDay * 24)).count());
	}

	SDateTime TimeUtil::ConvertSDateTimeFromString(const std::string& timeString) const
	{
		std::tm tm{};
	...
	SDateTime TimeUtil::ConvertKSTDateFromISO8601(const std::string& timeString) const
	{
		std::tm tm{};
		std::istringstream stream(timeString);
		SDateTime result = FindMinDateTime();
		...
		if (std::regex_match(timeString, regISO8601))
		{
			...
			result = ConvertKSTTimeMsToSDateTime(kstMS);
		}

		return result;
	}

	SDateTime TimeUtil::ConvertKSTDateFromISO8601WithMS(const std::string& timeString) const
	{
		std::tm tm{};
		std::istringstream stream(timeString);
		SDateTime result = FindMinDateTime();
	...
	/// <returns></returns>
	bool TimeUtil::IsInPeriod(const SDateTime& time1, const SDateTime& time2, const SDateTime& pivotTime) const
	{
		if (time1 < time2)
		{
	...
	SDateTime TimeUtil::AddSecondsSDateTimeToKSTDate(const SDateTime& time, const int64_t& addSeconds) const
	{
		...
		SDateTime result = ConvertTmToSDateTime(tm, time.Milliseconds);

		return result;
	}

	SDateTime TimeUtil::AddMinutesSDateTimeToKSTDate(const SDateTime& time, const int64_t& addMinutes) const
	{
		return AddSecondsSDateTimeToKSTDate(time, addMinutes * 60);
	}

	SDateTime TimeUtil::FindAfterSecondsSDateTimeToKSTDate(const int64_t& afterSeconds) const
	{
		return AddSecondsSDateTimeToKSTDate(FindKSTDate(), afterSeconds);
	}

	std::string TimeUtil::ToString(const SDateTime& time)
	{
		std::tm tm{};
	...
	std::string TimeUtil::ToISO8601Format(const SDateTime& time)
	{
	...
	int64_t TimeUtil::ConvertSDateTimeToKSTTimeMs(const SDateTime& time) const
	{
		return ConvertSDateTimeToUTCTimeMs(time) + g_KoreaTimeZoneMS;
	}
	...
	SDateTime TimeUtil::ConvertKSTTimeMsToSDateTime(const int64_t& timeMilli) const
	{
		...
		return ConvertTmToSDateTime(tm, timeMilli % g_SecToMs);
	}

	SDateTime TimeUtil::ConvertTmToSDateTime(const std::tm& tm, const int32_t& milliSec) const
	{
		SDateTime result{};
	...
	SDateTime TimeUtil::FindDateTimeByDayAndHour(const SDateTime& curDateTime, const int32_t& dayOfWeek, const int32_t& hour) const
	{
		SDateTime result{};

		const auto dayOffset = dayOfWeek - curDateTime.DayOfWeek;
		result = AddHourSDateTimeToKSTDate(curDateTime, dayOffset * g_oneDayHours);
	...
	SDateTime TimeUtil::FindNextDateTimeByHour(const SDateTime& curDateTime, const int32_t& hour) const
	{
		auto result = curDateTime;
		if (hour <= curDateTime.Hour)
		{
			result = AddHourSDateTimeToKSTDate(curDateTime, g_oneDayHours);
		}
	...
	SDateTime TimeUtil::FindNextDateTimeByDayAndHour(const SDateTime& curDateTime, const int32_t& dayOfWeek, const int32_t& hour) const
	{
		SDateTime result{};
		...
		{
			result = AddHourSDateTimeToKSTDate(result, g_MaxDayOfWeek * g_oneDayHours);
		}
	...
	int64_t TimeUtil::FindCorrectIntervalSeconds(const SDateTime& start, const SDateTime& end) const
	{
		const auto startEpochSeconds = FindEpochTime(start);
		const auto endEpochSeconds = FindEpochTime(end);
	...
	int64_t TimeUtil::FindCorrectIntervalDays(const SDateTime& start, const SDateTime& end) const
	{
	...
	int64_t TimeUtil::FindCorrectIntervalWeeklys(const SDateTime& start, const SDateTime& end) const
	{
	...
	int64_t TimeUtil::ConvertSDateTimeToUTCTimeMs(const SDateTime& time) const
	{
		return FindEpochTime(time) * g_SecToMs + time.Milliseconds;
	}

	SDateTime TimeUtil::ConvertKSTDateTimeToUTCDateTime(const SDateTime& kstDateTime) const
	{
		// kst - 9 = utc
		return AddHourSDateTimeToKSTDate(kstDateTime, static_cast<int32_t>(g_UTCToKSTOffset * -1));
	}
	...
	int64_t TimeUtil::FindEpochTime(const SDateTime& time) const
	{
		std::tm tm{};
		tm.tm_year = time.Year - g_InitializeYearValue;
		tm.tm_mon = time.Month - g_InitializeMonthValue;
	...
	SDateTime TimeUtil::FindTimeAfterRepeatedSeconds(const SDateTime& start, const int64_t& seconds) const
	{
		...
		return AddSecondsSDateTimeToKSTDate(start, (intervalSecs / seconds + 1) * seconds);
	}

	SDateTime TimeUtil::MakeScheduleTime(const SDateTime& defaultDateTime, const int32_t& addDay, const SDateTime& settingTime) const
	{
		if (settingTime.Year != 1900) return{};

		const auto& defaultMs = ConvertSDateTimeToKSTTimeMs(defaultDateTime);
		const auto& settingTimeMs = (settingTime.Hour * 3600 + settingTime.Minute * 60) * 1000;
		const auto& dayMs = addDay * 86400 * 1000;

		return ConvertKSTTimeMsToSDateTime(defaultMs + settingTimeMs + dayMs);
	}
	...
	SDateTime TimeUtil::FindLastDailyResetTime(const int32_t& resetHour) const
	{
		...
		SDateTime result;
		result = AddHourSDateTimeToKSTDate(truncateHourDateTime, resetHour);
		if (FindKSTDate() <= result)
		{
			result = AddHourSDateTimeToKSTDate(result, g_oneDayHours * -1);
		}

		return result;
	}

	SDateTime TimeUtil::FindLastWeeklyResetTime(const int32_t& resetDay, const int32_t& resetHour) const
	{
		auto truncateHourDateTime = FindKSTDate();
		// 일 밑으로 절삭
		truncateHourDateTime.Hour = 0;
		...
		// 차이 일수 만큼 시간을 빼줌
		auto lastResetDateTime = AddHourSDateTimeToKSTDate(truncateHourDateTime, compareDayOfWeekResult * g_oneDayHours * -1);

		// resetHour시간 더한 시간이 미래면 일주일 빼서 계산
		SDateTime result;
		result = AddHourSDateTimeToKSTDate(lastResetDateTime, resetHour);
		if (FindKSTDate() < result)
		{
			result = AddHourSDateTimeToKSTDate(result, g_oneDayHours * g_DaysPerWeek * -1);
		}

		return result;
	}

	SDateTime TimeUtil::FindLastMonthlyResetTime(const int32_t& resetDay, const int32_t& resetHour) const
	{
		...
		SDateTime result;
		result = AddHourSDateTimeToKSTDate(targetResetTime, resetHour);
		if (FindKSTDate() < result)
		{
	...
	SDateTime TimeUtil::FindNextDailyResetTime(const int64_t& curMs, const int32_t& resetHour) const
	{
		return FindNextDailyResetTime(ConvertKSTTimeMsToSDateTime(curMs), resetHour);
	}

	SDateTime TimeUtil::FindNextWeeklyResetTime(const int64_t& curMs, const int32_t& resetHour, const int32_t& resetDayOfWeek) const
	{
		return FindNextWeeklyResetTime(ConvertKSTTimeMsToSDateTime(curMs), resetDayOfWeek, resetHour);
	}

	SDateTime TimeUtil::FindNextMonthlyResetTime(const int64_t& curMs, const int32_t& resetHour, const int32_t& resetDay) const
	{
		return FindNextMonthlyResetTime(ConvertKSTTimeMsToSDateTime(curMs), resetDay, resetHour);
	}

	SDateTime TimeUtil::FindNextDailyResetTime(const SDateTime& lastUpdateTime, const int32_t& resetHour) const
	{
		auto truncateHourDateTime = lastUpdateTime.TruncateHoursAndLess();
		const auto& result = AddHourSDateTimeToKSTDate(truncateHourDateTime, resetHour);
		if (lastUpdateTime < result) return result;
		return AddHourSDateTimeToKSTDate(result, g_HoursPerDay);
	}

	SDateTime TimeUtil::FindNextWeeklyResetTime(const SDateTime& lastUpdateTime, const int32_t& resetDayOfWeek, const int32_t& resetHour) const
	{
		auto truncateHourDateTime = lastUpdateTime.TruncateHoursAndLess();
		...
		const auto& result = AddHourSDateTimeToKSTDate(truncateHourDateTime, diffDayOfWeek * g_HoursPerDay + resetHour);
		if (lastUpdateTime < result) return result;
		return AddHourSDateTimeToKSTDate(result, g_DaysPerWeek * g_HoursPerDay);
	}

	SDateTime TimeUtil::FindNextMonthlyResetTime(const SDateTime& lastUpdateTime, const int32_t& resetDay, const int32_t& resetHour) const
	{
	...
	SDateTime TimeUtil::FindLastDailyResetTime(const SDateTime& lastUpdateTime, const int32_t& resetHour) const
	{
		auto targetResetTime = lastUpdateTime.TruncateHoursAndLess();
		targetResetTime.Hour = static_cast<uint16_t>(resetHour);
		...
		if (timeDiff <= 0)
		{
			targetResetTime = AddHourSDateTimeToKSTDate(targetResetTime, -g_oneDayHours);
		}

		return targetResetTime;
	}

	SDateTime TimeUtil::FindLastWeeklyResetTime(const SDateTime& lastUpdateTime, const int32_t& resetDayOfWeek, const int32_t& resetHour) const
	{
		auto truncateHourDateTime = lastUpdateTime.TruncateHoursAndLess();
		auto diffDayOfWeek = lastUpdateTime.DayOfWeek - resetDayOfWeek;
		if (diffDayOfWeek < 0)
	...
	SDateTime TimeUtil::FindMinDateTime() const
	{
		return ConvertSDateTimeFromString("2000-01-01 00:00:00");
	}

	SDateTime TimeUtil::FindMaxDateTime() const
	{
		return ConvertSDateTimeFromString("3000-01-01 00:00:00");
	}

	int32_t TimeUtil::FindDayAfterNextResetHour(const int64_t& befMs, const int64_t& curMs, const int32_t& resetHour) const
	{
		const auto& nextDailyResetTime = FindNextDailyResetTime(befMs, resetHour);
		const auto& curDate = ConvertKSTTimeMsToSDateTime(curMs);
