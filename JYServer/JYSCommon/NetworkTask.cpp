...
namespace network
{
	...
	void NetworkTaskConnect::Prepare()
	{
		...
		{
			S_LOG_ERROR(host->GetHostID(), 0, "failed - WSASocket : [err:%]", WSAGetLastError());
			Failed();
			return;
		}
		...
		{
			S_LOG_ERROR(host->GetHostID(), 0, "SetSocket Failed");
			Failed();
			return;
		}
		...
		{
			S_LOG_ERROR(host->GetHostID(), 0, "failed - bind : [err:%]", WSAGetLastError());
			Failed();
			return;
		}
		...
		if (ConnectEx == nullptr)
		{
			...
			{
				S_LOG_ERROR(host->GetHostID(), 0, "WSAIoctl failed : [err:%]", WSAGetLastError());
				Failed();
				return;
			}
		}
		...
		if (ConnectEx(host->GetSocket(), (sockaddr*)&remote, sizeof(remote), nullptr, 0, nullptr, (LPOVERLAPPED)this) == FALSE)
		{
			...
			{
				S_LOG_ERROR(host->GetHostID(), 0, "ConnectEx failed : [err:%]", error);
				Failed();
				return;
			}
		}
	...
	void NetworkTaskConnect::Complete(bool result, int32_t transferred)
	{
		...
		if (result == false)
		{
			S_LOG_CRITICAL(host->GetHostID(), 0, "result failed");
			Failed();
			return;
		}
	...
	void NetworkTaskAccept::Prepare()
	{
		...
		{
			S_LOG_ERROR(host->GetHostID(), 0, "WSASocket failed : [err:%]", WSAGetLastError());
			Failed();
			return;
		}
		...
		if (AcceptEx(host->GetSocket(), m_socket, m_address, 0, sizeof(SOCKADDR_IN) + 16, sizeof(SOCKADDR_IN) + 16, &bytes, (LPOVERLAPPED)this) == FALSE)
		{
			...
			if (error != WSA_IO_PENDING)
			{
				S_LOG_ERROR(host->GetHostID(), 0, "AcceptEx failed : [err:%]", error);
				Failed();
				return;
			}
		}
	...
	void NetworkTaskReceive::Prepare()
	{
		...
		{
			S_LOG_ERROR(host->GetHostID(), 0, "ReadyToRecv failed");
			Failed();
			return;
		}
